...
class ReadableLocalCopyWriter {
  ...
  Future<void> appendRecord({
    ...
    final bucketDirectory = await _resolveBucketDirectory(
      root: root,
      sessionId: sessionId,
    );
    final filePath = p.join(bucketDirectory.path, '$stream.jsonl');
  ...
  Future<Directory> _resolveBucketDirectory({
    required Directory root,
    required String? sessionId,
  }) async {
    final normalized = sessionId?.trim();
    final targetPath = (normalized == null || normalized.isEmpty)
        ? p.join(root.path, 'global')
        : p.join(root.path, 'sessions', _normalizeSessionId(normalized));
